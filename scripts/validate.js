	...
	jFormValidator={
			  ...
			  'check_and_warning' : function (checked_id, min_length_value, data_type) {
					...
					if((pattern_not_cyrillic.test(checking_textbox.val())) && (data_type==jFormValidator.DIGITS_DENIED)){
									 ...
									 warn_div.removeClass('block_show_correct').addClass('block_show_error').html("← цифры недопустимы ").show();													
				...
				/*
				'check_file_photo' : function() {
												$('body').append('<div id="photoInfo" class="information_block"></div');
												var file_photo_info_div=$('#photoInfo');		
												var file_photo_element=$('#photo');
												var file_photo_element_position = file_photo_element.offset();		
												file_photo_info_div.css({
														top: file_photo_element_position.top-3,
														left: file_photo_element_position.left+file_photo_element.width()+15
												});		
												var photo_pathfile_pattern=/^[A-Z]{1}:\\([A-ZА-я.0-9!@()\\_\s:]*)$/i;
												if (!photo_pathfile_pattern.test(file_photo_element.val())){
													 jFormValidator.error_is_found=true;
													 file_photo_info_div.removeClass('block_show_correct').addClass('block_show_error').html("не соответствует шаблону").show();													
													 file_photo_element.removeClass('element_is_normal').addClass('element_is_wrong');
												} 
												else{
													file_photo_info_div.removeClass('block_show_error').addClass('block_show_correct').html("√").show();													
													 file_photo_element.removeClass('element_is_wrong').addClass('element_is_normal');
												}
				},
				*/
